...
- **상태:** 🔧 진행 중 — 방향을 **TDS(토스) 스타일**로 확정 (2026-06-18). 색(AppGrey 뼈대+시맨틱)·타이포(AppTypography 15/17축 스케일)·간격·반경 4종 토큰 확정, Theme·AppButton 연결, `gallery/showcase` 갤러리 앱으로 확인 가능. 남은 것: Pretendard 폰트 번들, 컴포넌트 확충(input·chip·dialog 등).
