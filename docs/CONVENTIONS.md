...
- **화면 배경은 흰색으로 고정.** 색은 배경이 아니라 **위젯이 짊어진다** — 채움(fill) 또는
  아웃라인(outline). 그래서 `BrandConfig`는 배경색을 갖지 않는다.
  - 특정 화면이 예외적으로 배경색이 필요하면 브랜드가 아니라 `AppScaffold(backgroundColor:)`로 개별 지정.
- **KeyColor(브랜드 seed)는 주역 위젯에만** 강하게. 화면당 브랜드색 큰 면적은 1~2개로 절제
  (예: D-Day의 히어로 카드 = 브랜드 채움 / 나머지 카드 = 흰 배경 + 브랜드 라벨).
